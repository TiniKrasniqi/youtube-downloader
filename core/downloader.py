...
import yt_dlp
from yt_dlp.utils import DownloadError, DownloadCancelled, ExtractorError
...
class YTAudioDownloader:
    ...
    def _download_playlist_concurrent(self, url: str, out_dir: str, bitrate: str) -> bool:
        meta_opts = {
            ...
            "skip_download": True,
            "logger": YTDLogger(self.log),
            # Let yt-dlp yield playlist entries lazily so we don't block on
            # fetching metadata for the entire list before starting downloads.
            "lazy_playlist": True,
        }
            ...
            with fetch_lock:
                while not self.stop_event.is_set():
                    try:
                        ...
                        error_event.set()
                        self.log(
                            f"[{human_time()}] ❌ Failed to fetch next playlist item: {e}"
                        )
                        self.progress(
                            DownloadProgress(
                                status="error",
